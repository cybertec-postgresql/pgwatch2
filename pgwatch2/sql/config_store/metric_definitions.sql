...
with recursive
    ...
    q_tstats as (
        ...
        WHERE NOT schemaname LIKE E'pg\\_temp%'
        AND (heap_blks_read > 0 OR heap_blks_hit > 0 OR idx_blks_read > 0 OR idx_blks_hit > 0 OR tidx_blks_read > 0 OR
           tidx_blks_hit > 0)
    )
select
    epoch_ns,
...
select * from (
    select
        ...
        quote_ident(qr.root_schema) || '.' || quote_ident(qr.root_relname) as tag_table_full_name,
        1 as is_part_root,
        sum(table_size_b) table_size_b,
        abs(greatest(ceil(log((sum(table_size_b) + 1) / 10 ^ 6)),
             0))::text as tag_table_size_cardinality_mb, -- i.e. 0=<1MB, 1=<10MB, 2=<100MB,..
        sum(total_relation_size_b) total_relation_size_b,
        sum(toast_size_b) toast_size_b,
        min(seconds_since_last_vacuum) seconds_since_last_vacuum,
        min(seconds_since_last_analyze) seconds_since_last_analyze,
        sum(no_autovacuum) no_autovacuum,
        sum(seq_scan) seq_scan,
        sum(seq_tup_read) seq_tup_read,
        sum(idx_scan) idx_scan,
        sum(idx_tup_fetch) idx_tup_fetch,
        sum(n_tup_ins) n_tup_ins,
        sum(n_tup_upd) n_tup_upd,
        sum(n_tup_del) n_tup_del,
        sum(n_tup_hot_upd) n_tup_hot_upd,
        sum(n_live_tup) n_live_tup,
        sum(n_dead_tup) n_dead_tup,
        ...
        max(tx_freeze_age) tx_freeze_age
      from
           q_tstats ts
           join q_parts qp on qp.relid = ts.relid
           join q_root_part qr on qr.oid = qp.root
